...
def insert_data_driverscontrol(connection, cursor, data, typematrix):
    try:
        cursor.executemany("""
            INSERT INTO api.i_driverscontrol (address1,zonename,categorytype,created, date1, description, endtime, i_bpartnername, i_flotaname, i_tractovalue, latitude, longitude, maxspeed, speedcategory, starttime, typematrix, waittime)
            VALUES (%(location)s, %(zonename)s, %(excesstype)s, %(created)s, %(controldate)s, %(description)s, %(endtime)s, %(driver)s, %(fleet)s, %(domain)s, %(latitude)s, %(longitude)s, %(speedmax)s, %(speed)s, %(starttime)s, %(typematrix)s, %(waittime)s);
        """, data)
        connection.commit()
    except Exception as e:
...
def insert_data_odometer(connection, cursor, data):
    ...
    query = "INSERT INTO api.testing_odometro(asset_ppu, odometer_amt, created) VALUES (%s, %s, %s);"
        ...
        if count == 0:
            ...
            for question in questions:
                ...
                try:
                    # Insertar los datos en la base de datos
                    cursor.execute(
                        "INSERT INTO api.i_checklist (report_id, report_date, driver_value, observation, asset_value, m_checklist_question_id, project_name, response, void, ramp, created) VALUES (%s,%s,%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                        (report_id, report_date, driver_rut, observation, domain, question_id, fleet, question_response, void, ramp, created))
                except psycopg2.Error as e:
                    logger.error(
