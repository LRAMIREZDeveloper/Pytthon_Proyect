...
excel_path = "list_data_asignados.xlsx"
ordered_data = []

url = f"{base_url}api/v1/tires/current-status/{inspection_type}?page=1"
...
if response.status_code == 200:
    ...
    for n in range(2, last_page + 1):
        ...
        url = f"{base_url}api/v1/tires/current-status/{inspection_type}?page={n}"
        response = requests.get(url, headers=headers)
