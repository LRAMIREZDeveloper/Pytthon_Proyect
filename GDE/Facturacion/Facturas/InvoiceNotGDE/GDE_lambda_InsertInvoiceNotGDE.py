...
from queries import DATA_CLIENT_SB_C, INSERT_FACCT, SELECT_FACCT

# Configuración del logging
logging.basicConfig(filename='GDE_log_InsertInvoiceNotGDE.log', level=logging.DEBUG, format='%(message)s')
...
def procesar_documentos_e_insertar(documentos):
    ...
    for doc in documentos:
        try:
            ...
            classification_facct = 'Pendiente - Factura no presenta detalle en GDE.'
            dif = 0
            ...
            if proveedor_info:
                ...
                if archive == 'False':
                    ...
                    if exists:
                        ...
                        cursor.execute(
                            ...
                            (
                                ...
                                clasificacion_oc, classification_facct, dif
                            )
                        )
                    ...
                    logger.info(f"Factura {folio} no es trabajada como consolidado.")
