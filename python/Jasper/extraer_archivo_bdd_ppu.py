...
def consult_data_and_save(ppu):
    ...
    try:
        ...
        cur.execute("""
            SELECT pp.value, cd.a_asset_id, at.binarydata
            FROM adempiere.C_CriticalDate cd
            JOIN adempiere.ad_attachment at ON at.record_id = cd.C_CriticalDate_id
            JOIN adempiere.a_asset pp ON pp.a_asset_id = cd.a_asset_id
            WHERE cd.a_asset_id = %s AND cd.status = 'AL'
            ...
            if binary_data:
                ...
                if file_type == 'zip':
                    ...
                    with zipfile.ZipFile(file_path, 'r') as zip_ref:
                        extract_dir = f'extracted_files_{patente}'
...
with connect_to_db() as conn:
    with conn.cursor() as cursor:
        # Ejecutar la consulta
        cursor.execute("SELECT a_asset_id FROM adempiere.a_asset WHERE ad_orgref_id = 1000067 AND isactive = 'Y'")
